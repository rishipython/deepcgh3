...
class DeepCGH(nn.Module):
  ...
  n: float = 1.0
  N_pad: int = 0
  initial_shape: tuple = (shape[1], shape[2], 1)

  def setup(self):
    self.encoder = DeepCGHEncoder()
    self.decoder = FourierDecoder(self.initial_shape,
  ...
  def __call__(self, xy_coords, z_coords, z_planes, train: bool):
    ...
    slm_phase = self.decoder(encoded, train)
    ...
    return (recon, slm_phase), (None, None, None)
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        ...
        (recon, slm_phase), (amp, phase, field) = state.apply_fn({'params': params},
                                        xy_coords = batch[0],
                                        z_coords = batch[1],
...
cnn = DeepCGH(conv_params=conv_params, fourier_features=fourier_features)
