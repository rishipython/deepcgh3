...
variables = cnn.init(init_rng,
                        jnp.ones((batch_size, n_points, 2)),
                        train=False)

params_to_use = 'params_epoch_380.pkl'
params = deserialize('../', params_to_use)

data = []
for step in range(10):
  batch = gen.get_batch(batch_size)
  # Run optimization steps over training batches and compute batch metrics
  slm_phase = cnn.apply({'params': params},
                                    inputs = batch[0],
                                    train=False)
  recon = jnp.square(jnp.abs(jnp.fft.fft2(jnp.exp(1j*slm_phase))))[:, None, ...]
  loss = loss_cosine(recon, batch[1])
...
serialize('', f'sample_data_for_{params_to_use}', data)
