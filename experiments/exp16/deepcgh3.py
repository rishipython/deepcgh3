...
class TransformerDecoder(nn.Module):
    ...
    def setup(self):
        self.input_projection = nn.Dense(self.embed_dim - 512)
        self.decoder_blocks = [TransformerDecoderBlock(
            embed_dim=self.embed_dim,
            num_heads=self.num_heads,
            dropout_rate=self.dropout_rate
        ) for _ in range(self.num_layers)]
        self.output_projection = nn.Dense(1)
        self.output_cleanup = nn.Dense(shape[1]*shape[2])

    def __call__(self, slm_points, encodings, train: bool = True):
        x = self.input_projection(slm_points)
        x = jnp.concatenate([x, encodings], axis=-1)
        for block in self.decoder_blocks:
            x = block(x, train)
        x = self.output_projection(x)
        x = self.output_cleanup(x[:,:,0])
        return x
...
class PointNetImageGen(nn.Module):
    ...
    def get_transformer_inputs(self, enc):
      slm_points = ScalarField.create(
            self.spacing,
            self.spectrum,
            self.spectral_density,
      ...
      slm_points = jnp.stack([slm_points[0].flatten(), slm_points[1].flatten()], axis=-1)
      repeated_enc = repeat(enc, f'a b -> a {slm_points.shape[0]} b')
      repeated_slm_points = repeat(slm_points, f"a b -> {enc.shape[0]} a b")
      return (repeated_slm_points, repeated_enc)
    ...
    def __call__(self, inputs, train: bool):
        ...
        transformer_inputs = self.get_transformer_inputs(encoded)
        decoded = self.decoder(*transformer_inputs)
...
def create_train_state(module, rng, learning_rate):
    ...
    # tx = optax.chain(
    #     optax.clip(1.0),           # Clip gradients with norm larger than 1.0
...
for step in range(num_steps_per_epoch*epochs):
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    ...
    pickle.dump(metrics_history, open(f'current_metrics_history.pkl', 'wb'))
    print('Saved metrics history to current_metrics_history.pkl')

    state = state.replace(metrics=state.metrics.empty()) # reset train_metrics for next training epoch

    if ((step+1) // num_steps_per_epoch) % 10 == 0:
      pickle.dump(state.params, open(f'params_epoch_{(step+1) // num_steps_per_epoch}.pkl', 'wb'))
      print(f'Saved params to params_epoch_{(step+1) // num_steps_per_epoch}.pkl')

      serialize('', f'state_opt_state_epoch_{(step+1) // num_steps_per_epoch}.pkl', state.opt_state)
...
# serialize('', f'state_{datetime}.pkl', state)
# print(f'state_{datetime}.pkl')

serialize('', f'state_opt_state_{datetime}.pkl', state.opt_state)
