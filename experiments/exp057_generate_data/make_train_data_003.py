...
save_path = f"/nrs/turaga/athavaler/experiments/{os.getcwd().split('/')[-1]}"
...
@jax.jit
def compute_metrics(*, state, batch, is_test=False):
...
for step in tqdm(range(num_steps_per_epoch*epochs)):
  batch = gen.get_batch(batch_size)
  batches.append(batch)
    
serialize(save_path, 'train_data_003.pkl', batches)