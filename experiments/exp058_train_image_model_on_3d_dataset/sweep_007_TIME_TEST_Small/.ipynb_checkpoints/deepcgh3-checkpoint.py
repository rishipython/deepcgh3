...
for step in range(num_steps_per_epoch*epochs):
  t0_total = t0 = time.time()
    
  times = {
      'batch': None,
      'train_step': None,
      'compute_metrics': None,
      'compute_test_metrics': None,
      'log': None
  }
      
  # batch = gen.get_batch(batch_size)
  batch = list(train_data[int(step % len(train_data))])
  batch[-1] = batch[-1][:,:,:128,:128]
  times['batch'] = time.time() - t0
  t0 = time.time()
  ...
  cache['grads'] = grads
  times['compute_metrics'] = time.time() - t0
  t0 = time.time()

  state, _ = compute_test_metrics(state=state, batch=test_batch)
  jax.tree_util.tree_leaves(state.params)[0].block_until_ready()
  times['compute_test_metrics'] = time.time() - t0
  t0 = time.time()
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    ...
    state = state.replace(test_metrics=state.test_metrics.empty())
  ...
  times['log'] = time.time() - t0

  times['total'] = time.time() - t0_total
  if step != 0: print(times)
