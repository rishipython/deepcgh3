...
save_path = '/nrs/turaga/athavaler/experiments/exp052_050fixed'
...
def normalize(x): # TODO
    ...
    y = x - jnp.array([0, 0, radius[0], intensity[0]])[None, None, ...]#, radius[0], intensity[0], z_range[0]])[None, None, ...]
    # y /= jnp.array([shape[2], shape[1], radius[1] - radius[0], intensity[1] - intensity[0]])[None, None, ...]
    y /= jnp.array([shape[2], shape[1], radius[1] - radius[0], intensity[1] - intensity[0]])[None, None, ...]#, radius[1] - radius[0], intensity[1] - intensity[0], z_range[1] - z_range[0]])[None, None, ...]
    y -= 0.5
    return y * 2
...
class DeepCGH(nn.Module):
  ...
  def setup(self):
    self.xy_encoder = DeepCGHEncoder()
    self.ri_encoder = DeepCGHEncoder()
    self.combine_encodings = nn.Sequential([
  ...
  def __call__(self, inputs, train: bool):
    ...
    xy_encoded = self.xy_encoder(xy_coords)
    ri_encoded = self.ri_encoder(ri_coords)

    encoded = jnp.concatenate([xy_encoded, ri_encoded], axis=-1)
    encoded = self.combine_encodings(encoded)
...
params_encoder = deserialize('/nrs/turaga/athavaler/experiments/exp005_larger_encoder_lr_schedule_relu_5_spots_amp_phase_output', 'params_epoch_92350.pkl')['encoder']

def create_train_state(module, rng, learning_rate):
    """Creates an initial `TrainState`."""
    # initialize parameters by passing a template image
    variables = module.init(rng,
                            jnp.ones((batch_size, n_points, 4)),# if shape[0] == 1 else 5)), # TODO
                            train=False)
    if starting_params_path is None:
        params = variables['params']
        del params["xy_encoder"]
    else:
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        slm_phase, updates = state.apply_fn({'params': {"xy_encoder": params_encoder, **params}},# 'batch_stats': state.batch_stats},
                                        inputs = batch[0],
                                        train=True,
                                        mutable=['batch_stats'])
...
def compute_metrics(*, state, batch):
    slm_phase = state.apply_fn({'params': {"xy_encoder": params_encoder, **state.params}},# 'batch_stats': state.batch_stats},
                                    inputs = batch[0],
                                    train=False)
    recon = forward(slm_phase)[:, None, ...]
...
assert("xy_encoder" not in state.params)
