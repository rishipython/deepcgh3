...
save_path = '/nrs/turaga/athavaler/experiments/exp024_higher_lr_on_encoder'
...
def create_train_state(module, rng, encoder_learning_rate, decoder_learning_rate):
    ...
    tx = optax.chain(
        optax.chain(
            optax.masked(optax.radam(encoder_learning_rate), encoder_mask),
            optax.contrib.reduce_on_plateau(patience=10000, cooldown=0, factor=0.5, accumulation_size=5000)
        ),
        optax.chain(
            optax.masked(optax.radam(decoder_learning_rate), decoder_mask),
            optax.contrib.reduce_on_plateau(patience=10000, cooldown=0, factor=0.5, accumulation_size=5000)
        )
    )
...
#%
state = create_train_state(cnn, init_rng, encoder_learning_rate, decoder_learning_rate)
