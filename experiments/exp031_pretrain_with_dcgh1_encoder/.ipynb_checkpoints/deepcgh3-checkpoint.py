...
save_path = '/nrs/turaga/athavaler/experiments/exp031_pretrain_with_dcgh1_encoder'
...
class DeepCGHImageEncoder(nn.Module):
    interleave_factor: int = 16

    @nn.compact
    def __call__(self, target: Array, train: bool):
        return self.get_encoding(target, "train" if train else "test")

    def get_encoding(self, target: Array, mode: str = "train") -> Array:
...
class DeepCGH(nn.Module):
  ...
  def setup(self):
    ...
    self.encoder = DeepCGHImageEncoder()
    self.decoder = DeepCGHDecoder()

  def get_image_from_phase(self, phase, z):
      propagator = jax.vmap(compute_transfer_propagator, in_axes=(None, 0, None))(
                plane_wave(self.slm_shape, self.spacing, self.spectrum, self.spectral_density),
                z,
                self.n,
            )
  ...
  def __call__(self, target, z_planes, train: bool):
    encoded = self.encoder(jnp.moveaxis(target, 1, -1), train)
...
def create_train_state(module, rng, learning_rate):
    ...
    variables = module.init(rng,
                            jnp.ones((batch_size, shape[0], shape[1], shape[2])),
                            jnp.ones((batch_size, shape[0])),
                            train=False)
    ...
    if starting_batch_stats_path is None:
        ...
        print('Using starting batch_stats')
...
def compute_metrics(*, state, batch):
    ...
    return state, {'recon': recon, 'slm_phase': slm_phase, 'batch': batch, 'amp': amp, 'phase': phase, 'field': field, 'batch_stats': state.batch_stats}
