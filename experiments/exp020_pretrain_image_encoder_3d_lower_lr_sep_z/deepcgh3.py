...
save_path = '/nrs/turaga/athavaler/experiments/exp020_pretrain_image_encoder_3d_lower_lr_sep_z'
...
class DeepCGH(nn.Module):
  ...
  def setup(self):
    self.xy_coords_encoder = DeepCGHEncoder()
    self.z_coords_encoder = DeepCGHEncoder()
  ...
  def __call__(self, xy_coords, z_coords, z_planes, train: bool):
    ...
    xy_encoded = self.xy_coords_encoder(xy_coords)
    z_encoded = self.z_coords_encoder(z_coords)
    
    encoded = jnp.concatenate([xy_encoded, z_encoded], axis=-1)
