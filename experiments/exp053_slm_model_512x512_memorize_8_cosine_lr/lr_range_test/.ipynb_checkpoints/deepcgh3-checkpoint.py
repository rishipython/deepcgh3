...
starting_lr = 1e-8
ending_lr = 1e-3
lrs = jnp.linspace(starting_lr, ending_lr, epochs)
...
def create_train_state(module, rng, starting_lr):
    ...
    tx = optax.inject_hyperparams(optax.radam)(learning_rate=starting_lr)
...
def compute_metrics(*, state, batch):
    ...
    metric_updates = state.train_metrics.single_from_model_output(predictions = recon, targets=batch[3], loss_cosine=loss['loss_cosine'], loss_acc=loss['loss_acc'])
    metrics = state.train_metrics.merge(metric_updates)
    state = state.replace(train_metrics=metrics)
...
#%
state = create_train_state(cnn, init_rng, starting_lr)
...
train_data = deserialize(save_path, 'train_data.pkl')
...
for step in range(num_steps_per_epoch*epochs):
  ...
  state, _ = compute_test_metrics(state=state, batch=test_batch) # aggregate batch metrics

  if step == 0:
      norm_ratios = jax.tree_util.tree_map(lambda g, p: jnp.linalg.norm(g / p), grads, starting_params)
      print("Norm ratios:")
      print(norm_ratios)
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    ...
    state = state.replace(train_metrics=state.train_metrics.empty()) # reset train_metrics for next training epoch
    state = state.replace(test_metrics=state.test_metrics.empty()) # reset train_metrics for next training epoch
