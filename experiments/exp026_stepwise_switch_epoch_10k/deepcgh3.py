...
save_path = '/nrs/turaga/athavaler/experiments/exp026_stepwise_switch_epoch_10k'
...
def create_train_state(module, rng, encoder_learning_rate, decoder_learning_rate, switch_epoch):
    ...
    def stepwise_schedule(epoch, switch_epoch, encoder_lr, decoder_lr):
        return (1 - ((epoch // switch_epoch) % 2 == 0)) * encoder_lr, ((epoch // switch_epoch) % 2 == 0) * decoder_lr

    def stepwise_scheduler(switch_epoch, encoder_lr, decoder_lr):
        def schedule_fn(epoch):
            encoder_learning_rate, decoder_learning_rate = stepwise_schedule(epoch, switch_epoch, encoder_lr, decoder_lr)
            return {'encoder_lr': encoder_learning_rate, 'decoder_lr': decoder_learning_rate}
        
        return schedule_fn

    learning_rate_schedule = stepwise_scheduler(switch_epoch, encoder_learning_rate, decoder_learning_rate)

    encoder_mask = {"encoder": jax.tree_util.tree_map(lambda x: True, params["encoder"]), "decoder": jax.tree_util.tree_map(lambda x: False, params["decoder"])}

    decoder_mask = {"encoder": jax.tree_util.tree_map(lambda x: False, params["encoder"]), "decoder": jax.tree_util.tree_map(lambda x: True, params["decoder"])}
    
    tx = optax.chain(
        optax.chain(
            optax.masked(optax.radam(lambda step: learning_rate_schedule(step)['encoder_lr']), encoder_mask),
            optax.contrib.reduce_on_plateau(patience=10000, cooldown=0, factor=0.5, accumulation_size=5000)
        ),
        optax.chain(
            optax.masked(optax.radam(lambda step: learning_rate_schedule(step)['decoder_lr']), decoder_mask),
            optax.contrib.reduce_on_plateau(patience=10000, cooldown=0, factor=0.5, accumulation_size=5000)
        )
    )
...
#%
state = create_train_state(cnn, init_rng, encoder_learning_rate, decoder_learning_rate, switch_epoch)
