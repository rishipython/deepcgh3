...
data_path = '/nrs/turaga/athavaler/experiments/exp008_deepcgh1'
...
batch_size = 1
dcgh1 = DeepCGH(shape=(size, size), interleave_factor=size // 32)
# variables = jax.pmap(dcgh1.init)(
#     repeat(jax.random.PRNGKey(3331), "k -> d k", d=num_devices),
#     jnp.zeros((num_devices, batch_size // num_devices, size, size, 1))
# )
variables = dcgh1.init(jax.random.PRNGKey(3331), jnp.zeros((batch_size, size, size, 1)))
...
# print()
# print('var bs')
# print(jax.tree_util.tree_map(lambda x: x.shape, variables["batch_stats"]))

def deserialize(save_path: str, fname: str) -> Any:
    with open(os.path.join(save_path, fname), 'rb') as f:
        return pickle.load(f)

dcgh1_params = deserialize(data_path, 'params.pkl')
# dcgh1_state = variables["state"]
# batch_stats = variables["batch_stats"]
dcgh1_batch_stats = deserialize(data_path, 'batch_stats.pkl')

# print()
# print()
# print()
# print('load params')
# print(jax.tree_util.tree_map(lambda x: x.shape, dcgh1_params))
# print()
# print()
# print()
# print('load bs')
# print(jax.tree_util.tree_map(lambda x: x.shape, dcgh1_batch_stats))

dcgh1_params = jax.tree_util.tree_map(lambda x: x[0], dcgh1_params)
dcgh1_batch_stats = jax.tree_util.tree_map(lambda x: x[0], dcgh1_batch_stats)

# @partial(jax.pmap, axis_name="devices")
@jax.jit
def run_dcgh1(batch_label):
    target = batch_label[...,None]
    target = target[0]
    approx = dcgh1.apply({"params": dcgh1_params, "batch_stats": dcgh1_batch_stats}, target, mode="test")
    return approx[None,...]
...
save_path = '/nrs/turaga/athavaler/experiments/exp014_lower_LR'

if not os.path.exists(save_path):
    os.makedirs(save_path)

starting_params_path = '/nrs/turaga/athavaler/experiments/exp013_pretrain_to_100_dcgh1_outputs/params_epoch_2000.pkl'
starting_opt_state_path = '/nrs/turaga/athavaler/experiments/exp013_pretrain_to_100_dcgh1_outputs/state_opt_state_epoch_2000.pkl'
...
N = 100
n_points = 5 # 50
num_steps_per_epoch = 100 #N // batch_size
radius = 5#[5, 25]
intensity = 1#[1, 5]
...
test_gen = RandoDotGenerator(N = 100,
                        n_points = n_points,
                        radius = radius,
                        intensity = intensity,
                        shape = shape,
                        ...
                        output_arguments = {'x': True,
                                            ...
                                            'intensity': False,
                                            'image': True,
                                            'pixel_weights': False}, refresh=True)
...
class Metrics(metrics.Collection):
  ...
  loss_mse: metrics.Average.from_output('loss_mse')

  test_loss_cosine: metrics.Average.from_output('test_loss_cosine')
  test_loss_acc: metrics.Average.from_output('test_loss_acc')
  test_loss_mse: metrics.Average.from_output('test_loss_mse')
...
def train_step(state: TrainState, batch):
    target_slm_phase = run_dcgh1(batch[1])
    ...
    def loss_fn(params):
        ...
        loss = loss_mse(slm_phase, target_slm_phase)
...
@jax.jit
def compute_metrics(*, state, batch, test_batch):
    ...
    target_slm_phase = run_dcgh1(batch[1])
    target_recon = forward(target_slm_phase)[:, None, ...]

    loss = {
        'loss_cosine': loss_cosine(recon, batch[1]),
        'loss_acc': loss_acc(recon, batch[1]),
        'loss_mse': loss_mse(slm_phase, target_slm_phase)
    }

    test_slm_phase = state.apply_fn({'params': state.params},# 'batch_stats': state.batch_stats},
                                    inputs = test_batch[0],
                                    train=False)
    test_recon = forward(test_slm_phase)[:, None, ...]

    test_target_slm_phase = run_dcgh1(test_batch[1])
    test_target_recon = forward(test_target_slm_phase)[:, None, ...]

    test_loss = {
        'loss_cosine': loss_cosine(test_recon, test_batch[1]),
        'loss_acc': loss_acc(test_recon, test_batch[1]),
        'loss_mse': loss_mse(test_slm_phase, test_target_slm_phase)
    }

    metric_updates = state.metrics.single_from_model_output(predictions = recon, targets=batch[1], loss_cosine=loss['loss_cosine'], loss_acc=loss['loss_acc'], loss_mse=loss['loss_mse'], test_loss_cosine=test_loss['loss_cosine'], test_loss_acc=test_loss['loss_acc'], test_loss_mse=test_loss['loss_mse'])
    
    metrics = state.metrics.merge(metric_updates)
    state = state.replace(metrics=metrics)
    return state, {'recon': recon, 'slm_phase': slm_phase, 'batch': batch, 'target_slm_phase': target_slm_phase, 'target_recon': target_recon, 'test_recon': test_recon, 'test_slm_phase': test_slm_phase, 'test_batch': test_batch, 'test_target_slm_phase': test_target_slm_phase, 'test_target_recon': test_target_recon}
...
for step in range(num_steps_per_epoch*epochs):
  ...
  test_batch = test_gen.get_batch(1)
  state, cache = compute_metrics(state=state, batch=batch, test_batch=test_batch) # aggregate batch metrics
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    for metric,value in state.metrics.compute().items(): # compute metrics
      if 'test' in metric: metrics_history[f'{metric}'].append(value)
      else: metrics_history[f'train_{metric}'].append(value) # record metrics
    ...
    print(f"train epoch: {(step+1) // num_steps_per_epoch}, "
          ...
          f"loss_mse: {metrics_history['train_loss_mse'][-1]}, "
          )
    print(f"test epoch: {(step+1) // num_steps_per_epoch}, "
          f"loss_acc: {metrics_history['test_loss_acc'][-1]}, "
          f"loss_cosine: {metrics_history['test_loss_cosine'][-1]}, "
          f"loss_mse: {metrics_history['test_loss_mse'][-1]}, "
          )
