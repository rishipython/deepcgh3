...
class DeepCGHDecoder(nn.Module):
  ...
  def setup(self):
    self.inr_linear_layers = [nn.Dense(f, dtype=self.dtype, precision=self.precision) for f in [self.layer_size for i in range(self.num_layers)]]
...
params_encoder = deserialize('', 'dcgh2/params_current.pkl')['encoder']
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        ((recon, slm_phase), (amp, phase, field)), updates = state.apply_fn({'params': {"encoder": params_encoder, "decoder": params}},# 'batch_stats': state.batch_stats},
                                        xy_coords = batch[0],
                                        z_coords = batch[1],
...
def compute_metrics(*, state, batch):
    (recon, slm_phase), (amp, phase, field) = state.apply_fn({'params': {"encoder": params_encoder, "decoder": state.params}},# 'batch_stats': state.batch_stats},
                                    xy_coords = batch[0],
                                    z_coords = batch[1],
                                    z_planes = np.array(z_planes),
                                    train=False)
