...
embed_size = 1024 if 'embedsize' not in sweep_params else sweep_params['embedsize']
embed_size = int(embed_size)

fixed_training_set = True if 'fixedtrainingset' not in sweep_params else sweep_params['fixedtrainingset']

fixed_radius_and_intensity = True if 'fixedrandi' not in sweep_params else sweep_params['fixedrandi']
...
starting_params_path = f"/nrs/turaga/athavaler/experiments/{os.getcwd().split('/')[-2]}/{os.getcwd().split('/')[-1]}/params_epoch_129000.pkl"

starting_opt_state_path = f"/nrs/turaga/athavaler/experiments/{os.getcwd().split('/')[-2]}/{os.getcwd().split('/')[-1]}/state_opt_state_epoch_129000.pkl"

starting_batch_stats_path = f"/nrs/turaga/athavaler/experiments/{os.getcwd().split('/')[-2]}/{os.getcwd().split('/')[-1]}/batch_stats_epoch_129000.pkl"
...
def normalize(x): # TODO
    if x.shape[-1] == 3:
        ...
        y = x - jnp.array([0, 0])[None, None, ...]
        y /= jnp.array([shape[2], shape[1]])[None, None, ...]
    y -= 0.5
    return y * 2
...
class DeepCGH(nn.Module):
  spacing: float = 9.2e-6 # spacing at SLM plane
  # field of view = field.dx * number of pixels (512) = field.dx (=4.4157612e-05) * 512 = 0.0226087
  spectrum: Array = 1.04e-6 # wavelength, 1.04 microns
  spectral_density: Array = 1.0
  slm_shape: tuple = (shape[1], shape[2]) # number of pixels in SLM (512, 512)
  bit_depth: int = 8
  f: float = 0.2 # focal length
  n: float = 1.0
  N_pad: int = 0
  ...
  def __call__(self, xy_coords, z_coords, z_planes, train: bool):
    ...
    # inputs = jnp.concatenate([normalize(xy_coords), z_coords], axis=-1)
...
def compute_test_metrics(*, state, batch):
    ...
    metric_updates = state.test_metrics.single_from_model_output(predictions = recon, targets=batch[3], loss_cosine=loss['loss_cosine'], loss_acc=loss['loss_acc'])
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    ...
    if ((step+1) // num_steps_per_epoch) % 1000 == 0:
      for test_batch_index in range(100):
        test_batch = test_gen.get_batch(batch_size)
        state, cache_test = compute_test_metrics(state=state, batch=test_batch)
        # cache[f'cache_test_{test_batch_index}'] = cache_test
