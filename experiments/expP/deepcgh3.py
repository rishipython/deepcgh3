...
save_path = '/nrs/turaga/athavaler/experiments/expP'
...
class DeepCGH(nn.Module):
    ...
    def spot_coordinates_to_phase(self, spot_coordinates: Array) -> Array:
        ...
        inr_linear_layers = [nn.Dense(f) for f in [256, 128, 64, 32, 16, 1]]
        ...
        def implicit_neural_representation(spot_embedding: Array, slm_pixel_coordinate: Array) -> Array:
            ...
            for linear, frequency_shift in zip(inr_linear_layers, inr_frequency_shifts):
                frequency = frequency_shift[:, 0, jnp.newaxis]
                shift = frequency_shift[:, 1, jnp.newaxis]
                embedding = jnp.sin(frequency * linear(embedding) + shift)
            return embedding
