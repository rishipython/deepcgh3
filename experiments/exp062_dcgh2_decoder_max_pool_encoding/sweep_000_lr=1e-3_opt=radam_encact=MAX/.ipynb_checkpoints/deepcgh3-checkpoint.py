...
LR = sweep_params['lr']
optimizer = sweep_params['opt']
use_max = sweep_params['encact']
...
test_gen = RandoDotGenerator(N = N,
                        n_points = n_points,
                        radius = radius,
                        intensity = intensity,
                        shape = shape,
...
for step in range(num_steps_per_epoch*epochs):
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    ...
    if step < 100 or ((step+1) // num_steps_per_epoch) % 1000 == 0:
    ...
    print(f"train epoch: {(step+1) // num_steps_per_epoch}, "
          ...
          f"test_loss_acc: {metrics_history['test_loss_acc'][-1]}, "
          f"test_loss_cosine: {metrics_history['test_loss_cosine'][-1]}, "
          )
