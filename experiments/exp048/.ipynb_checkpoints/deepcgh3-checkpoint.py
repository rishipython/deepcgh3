...
save_path = '/nrs/turaga/athavaler/experiments/exp048'
...
def loss_mse(predictions, targets, penalty_factor=16384.0):
    predictions = jnp.clip(predictions, a_min=0)
    predictions = predictions / predictions.max()

    # loss = jnp.square(predictions - targets)
    # penalty_mask = predictions > targets
    # loss = jnp.where(penalty_mask, loss * penalty_factor, loss)
    loss = jnp.clip(predictions - targets, a_min=0)
    
    return jnp.mean(loss * penalty_factor)
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        ...
        loss = loss_mse(recon, batch[3]) + loss_cosine(recon, batch[3])
...
def compute_metrics(*, state, batch):
    ...
    loss = {
        ...
        'loss_mse': loss_mse(recon, batch[3])
    }

    metric_updates = state.metrics.single_from_model_output(predictions = recon, targets=batch[3], loss_cosine=loss['loss_cosine'], loss_acc=loss['loss_acc'], loss_mse=loss['loss_mse'])
