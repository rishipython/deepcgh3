...
save_path = '/nrs/turaga/athavaler/experiments/expV'
...
class DeepCGHEncoder(nn.Module):
  @nn.compact
  def __call__(self, x):
    embedding = nn.Dense(features=512)(x)
    embedding = nn.activation.leaky_relu(embedding)
...
class DeepCGHDecoder(nn.Module):
  ...
  def setup(self):
    self.inr_linear_layers = [nn.Dense(f, dtype=self.dtype, precision=self.precision) for f in [256, 128, 64, 32, 16]]
...
for step in range(num_steps_per_epoch*epochs):
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    ...
    if ((step+1) // num_steps_per_epoch) % 50 == 0:
      ...
      print(f'Saved cache (recon, slm phase, batch, and grads) to cache_epoch_{(step+1) // num_steps_per_epoch}.pkl')
      ...
      print(f'Saved cache (recon, slm phase, batch, and grads) to cache_current.pkl')
