...
save_path = '/nrs/turaga/athavaler/experiments/exp016_slm_model_with_image_encoder'

if not os.path.exists(save_path):
    os.makedirs(save_path)

starting_params_path = 'params_current_saved.pkl'
starting_opt_state_path = 'state_opt_state_current_saved.pkl'
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        slm_phase, updates = state.apply_fn({'params': {"encoder": params_encoder, "decoder": params}},# 'batch_stats': state.batch_stats},
                                        inputs = batch[0],
                                        train=True,
                                        mutable=['batch_stats'])
...
def compute_metrics(*, state, batch):
    slm_phase = state.apply_fn({'params': {"encoder": params_encoder, "decoder": state.params}},# 'batch_stats': state.batch_stats},
                                    inputs = batch[0],
                                    train=False)
    recon = forward(slm_phase)[:, None, ...]
