...
class RandoDotGenerator:
    ...
    def get_batch(self, batch_size):
      ...
      for i in range(batch_size):
        ...
        if self.num_planes > 1:
          Z_list = data[1].tolist()
          Z_arr = []
          for z in Z_list:
              # assert z in self.z_fixed, f"z value of {z} not in {self.z_fixed}"
              Z_arr.append(one_hot_encode(z, self.z_fixed))
          batch_z.append(Z_arr)
...
class DeepCGH(nn.Module):
  ...
  def __call__(self, xy_coords, z_coords, z_planes, train: bool):
    # inputs = jnp.concatenate([xy_coords, z_coords], axis=-1)
    # inputs = normalize(inputs)
...
def create_train_state(module, rng, learning_rate):
    ...
    variables = module.init(rng,
                            ...
                            jnp.ones((batch_size, n_points, 3)),
