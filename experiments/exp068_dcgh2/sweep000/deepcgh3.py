...
fixed_radius_and_intensity = True
...
class DeepCGH(nn.Module):
  ...
  encoder_channels: tuple = (64, 128, 512, 1024, 2048)
  initial_shape: tuple = (8, 8, 2048)

  def setup(self):
    self.channels = get_coord_channels((1, 128, 128))
    # self.encoder = DeepCGHEncoder()
    self.encoder = PointNetEncoder(list(self.encoder_channels))
  ...
  def __call__(self, xy_coords, z_coords, z_planes, train: bool):
    ...
    encoded = self.encoder(inputs, train)
