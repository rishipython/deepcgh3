...
save_path = '/nrs/turaga/athavaler/experiments/exp011_pretrain_to_dcgh1_output_LR_1e_5'
...
def compute_metrics(*, state, batch):
    ...
    metric_updates = state.metrics.single_from_model_output(predictions = recon, targets=batch[1], loss_cosine=loss['loss_cosine'], loss_acc=loss['loss_acc'], loss_mse=loss['loss_mse'])
    metrics = state.metrics.merge(metric_updates)
    state = state.replace(metrics=metrics)
    return state, {'recon': recon, 'slm_phase': slm_phase, 'batch': batch, 'target_slm_phase': target_slm_phase, 'target_recon': target_recon}
