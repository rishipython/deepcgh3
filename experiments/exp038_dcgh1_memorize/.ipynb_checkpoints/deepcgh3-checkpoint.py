...
save_path = '/nrs/turaga/athavaler/experiments/exp038_dcgh1_memorize'
...
class DeepCGH(nn.Module):
  ...
  def __call__(self, target, z_planes, train: bool):
    slm_phase = self.target_to_phase(jnp.moveaxis(target, 1, -1), "train" if train else "test")

    recon = self.get_image_from_phase(slm_phase, z_planes)
    
    return recon, slm_phase
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        (recon, slm_phase), updates = state.apply_fn({'params': params, 'batch_stats': state.batch_stats},
                                        target = batch[3],
...
def compute_metrics(*, state, batch):
    (recon, slm_phase) = state.apply_fn({'params': state.params, 'batch_stats': state.batch_stats},
                                    target = batch[3],
                                    z_planes = np.array(z_planes),
                                    train=False)
    loss = {
    ...
    return state, {'recon': recon, 'slm_phase': slm_phase, 'batch': batch, 'batch_stats': state.batch_stats}
