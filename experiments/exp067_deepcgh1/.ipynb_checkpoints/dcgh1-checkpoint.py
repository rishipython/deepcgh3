...
dataset_path = f"/nrs/turaga/athavaler/experiments/{folder_name}/_SHP(512, 512, 3)_N2560_SZ15_INT1_Crowd[10, 11]_CNTFalse/train.zarr"
assert dataset_path is not None, "Dataset path is None"
...
batch_size = 16
model = DeepCGH(shape=(size, size))
variables = jax.pmap(model.init)(
    repeat(jax.random.PRNGKey(6174), "k -> d k", d=num_devices),
...
dataset = zarr.load(dataset_path)
