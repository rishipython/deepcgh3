...
save_path = '/nrs/turaga/athavaler/experiments/expU'
...
def loss_cosine(predictions, targets):
    predictions = jnp.clip(predictions, min=0)
...
def loss_acc(predictions, targets):
    ...
    denominator = jnp.sqrt(jnp.sum(predictions**2, axis=(1, 2, 3)) * jnp.sum(targets[:,0,:,:,jnp.newaxis]**2, axis=(1, 2, 3)))
    loss = 1.0 - jnp.mean((jnp.sum(predictions * targets[:,0,:,:,jnp.newaxis], axis=(1, 2, 3)) + 1) / (denominator + 1), axis=0)
    return loss

loss_func = loss_cosine
...
def default_z():
    return 0.05 * jnp.array([0.0])

class DeepCGH(nn.Module):
    shape: Tuple[int, int] = (128, 128)
    spacing: float = 9.2e-6
    z: Array = field(default_factory=default_z) 
    f: float = 0.2
    n: float = 1.0
    ...
    @nn.compact
    def __call__(self, spot_coordinates: Array, train: bool) -> Field:
        phase = self.spot_coordinates_to_phase(spot_coordinates)
    ...
    def spot_coordinates_to_phase(self, spot_coordinates: Array) -> Array:
        def encoder_block(embedding: Array, qkv_features: int = 512, num_heads: int = 4) -> Array:
            ...
            embedding = embedding + nn.leaky_relu(nn.Dense(features=512)(embedding))
            embedding = nn.LayerNorm()(embedding)
            return embedding

        def encoder(spot_coordinates: Array) -> Array:
            embedding = nn.leaky_relu(nn.Dense(features=512)(spot_coordinates))
            for i in range(6):
                embedding = encoder_block(embedding)
            return jnp.mean(embedding, axis=-2)

        spot_embedding = encoder(spot_coordinates)

        inr_linear_layers = [nn.Dense(f, kernel_init=nn.initializers.variance_scaling(scale=6.0, mode='fan_in', distribution="uniform")) for f in [512, 512, 512, 512, 1]]
        inr_frequency_shifts = [nn.sigmoid(nn.Dense(2, kernel_init=nn.initializers.variance_scaling(scale=6.0, mode='fan_in', distribution="uniform"))(spot_embedding)) * 2.0 * jnp.pi for f in range(len(inr_linear_layers))]

        def positional_encoding(inputs, base: float = 2.0):
            inputs_freq = jax.vmap(
                lambda x: inputs * base ** x
            )(jnp.arange(self.positional_encoding_dims))
            periodic_fns = jnp.stack([jnp.sin(inputs_freq), jnp.cos(inputs_freq)])
        ...
        def implicit_neural_representation(spot_embedding: Array, slm_pixel_coordinate: Array) -> Array:
            slm_encodings = repeat(slm_pixel_coordinate, "c -> b c", b=spot_embedding.shape[0])
            if self.apply_positional_encoding: slm_encodings = positional_encoding(slm_encodings)
            ...
            for i, (linear, frequency_shift) in enumerate(zip(inr_linear_layers, inr_frequency_shifts)):
                frequency = frequency_shift[:, 0, jnp.newaxis]
                shift = frequency_shift[:, 1, jnp.newaxis]
                if i == 0: embedding = jnp.sin(30.0 * frequency * linear(embedding) + shift)
                else: embedding = jnp.sin(30.0 * frequency * linear(embedding) + shift)
            return embedding

        def decoder(spot_embedding: Array, slm_pixel_coordinates: Array) -> Array:
            phase = implicit_neural_representation(spot_embedding, slm_pixel_coordinates)
            phase = rearrange(phase, "b (h w) 1 -> b h w", h=self.shape[0], w=self.shape[1])
            return phase

        def _create_slm_pixel_coordinates() -> Array:
            grid = [
                jnp.linspace(
                    -self.shape[i] // 2, self.shape[i] // 2 - 1, num=self.shape[i]
                )
                for i in range(2)
            ]
            grid = self.spacing * (jnp.array(grid) + 0.5)
            grid = jnp.array(jnp.meshgrid(*grid, indexing="ij"))
            grid = rearrange(grid, "n h w -> (h w) n")
            grid = grid / grid.max()
            return grid

        slm_pixel_coordinates = self.variable(
            "state",
            "slm_pixel_coordinates",
            _create_slm_pixel_coordinates
        ).value
        phase = decoder(spot_embedding, slm_pixel_coordinates)
...
def forward(slm_phase):
    recon = jnp.exp(1j*(slm_phase % (2.0*jnp.pi)))
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        (field, slm_phase), updates = state.apply_fn({'params': state.params, 'state': state.state},
                                        batch[0],
                                        train=True,
                                        mutable=['batch_stats'])

        recon = field.intensity.squeeze(axis=(-2,-1))
        loss = loss_func(recon, batch[1])
...
def compute_metrics(*, state, batch):
    (field, slm_phase) = state.apply_fn({'params': state.params, 'state': state.state},
                                    batch[0],
                                    train=False)
    recon = field.intensity.squeeze(axis=(-2,-1))
    ...
    return state, {'recon': recon, 'slm_phase': slm_phase, 'batch': batch}
