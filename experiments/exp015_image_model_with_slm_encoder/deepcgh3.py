...
save_path = '/nrs/turaga/athavaler/experiments/exp015_image_model_with_slm_encoder'
...
params_encoder = deserialize('/nrs/turaga/athavaler/experiments/exp004_larger_encoder_lr_schedule_relu_5_spots', 'params_epoch_88100.pkl')['encoder']
...
def train_step(state: TrainState, batch):
    ...
    def loss_fn(params):
        (slm_phase, (amp, phase, field)), updates = state.apply_fn({'params': {"encoder": params_encoder, "decoder": params}},# 'batch_stats': state.batch_stats},
                                        inputs = batch[0],
                                        train=True,
                                        mutable=['batch_stats'])
...
def compute_metrics(*, state, batch):
    slm_phase, (amp, phase, field) = state.apply_fn({'params': {"encoder": params_encoder, "decoder": state.params}},# 'batch_stats': state.batch_stats},
                                    inputs = batch[0],
                                    train=False)
    recon = forward(slm_phase)[:, None, ...]
