...
def parse_params(sweep_string):
    # Split the string by underscores while keeping the key-value pairs intact
    parts = sweep_string.split('_')
    ...
    for part in parts:
        if '=' in part:
            ...
            if key == 'opt':
                value = getattr(optax, value)
            elif key == 'encact':
                value = bool(value == 'MAX')
            elif key == 'fixedtrainingset':
                value = bool(value == 'T')
            elif key == 'fixedrandi':
                value = bool(value == 'T')
            elif key == 'fixednumspots':
                value = bool(value == 'T')
            else:
                value = float(value)
...
sweep_folder_name = os.getcwd().split('/')[-1]
sweep_params = parse_params(sweep_folder_name)

print(sweep_params)

LR = 1e-5 if 'lr' not in sweep_params else sweep_params['lr']
optimizer = optax.radam if 'opt' not in sweep_params else sweep_params['opt']
use_max = False if 'encact' not in sweep_params else sweep_params['encact']
embed_size = 2048 #if 'embedsize' not in sweep_params else sweep_params['embedsize']
embed_size = int(embed_size)

layer_size = 128 if 'layersize' not in sweep_params else int(sweep_params['layersize'])
num_layers = 5 if 'numlayers' not in sweep_params else int(sweep_params['numlayers'])

fixed_training_set = False if 'fixedtrainingset' not in sweep_params else sweep_params['fixedtrainingset']

fixed_radius_and_intensity = False if 'fixedrandi' not in sweep_params else sweep_params['fixedrandi']

fixed_num_points = False if 'fixednumspots' not in sweep_params else sweep_params['fixednumspots']

# fourier_features = int(sweep_params['ff'])
# conv_params = [
#     {'ch': 8, 'ks': (11, 11)},
#     {'ch': 4, 'ks': (6, 6)},
#     {'ch': 2, 'ks': (3, 3)},
...
starting_batch_stats_path = None
...
# def one_hot_encode(value, arr):
#     indices = np.where(np.array(arr) == value)[0]
#     encoding = np.zeros_like(arr)
#     encoding[indices] = 1.0
#     return encoding
...
class RandoDotGenerator:
    ...
    def __get_randomized_coords(self):
        if isinstance(self.n_points, list): max_n_points = max(self.n_points)
        else: max_n_points = self.n_points

        if isinstance(self.radius, list):
            self.radii = np.random.randint(self.radius[0], self.radius[1], size = (self.N, max_n_points))
        else:
            self.radii = np.ones((self.N, max_n_points), dtype=np.uint8) * self.radius

        if isinstance(self.disk_intensity, list):
            self.disk_intensities = np.random.randint(self.disk_intensity[0], self.disk_intensity[1], size=(self.N, max_n_points))
        else:
            self.disk_intensities = np.ones((self.N, max_n_points), dtype=np.uint8) * self.disk_intensity

        self.get_pixel_weights()

        self.centery = np.empty((self.N, max_n_points), dtype=np.int32)
        self.centerx = np.empty((self.N, max_n_points), dtype=np.int32)
    ...
    def __getitem__(self, idx, num_points=None):
        if num_points is None:
            if isinstance(self.n_points, list): num_points = max(self.n_points)
            else: num_points = self.n_points

        coords = np.array([self.centerx[idx,:num_points], self.centery[idx,:num_points]]).T

        vector_elements = {}
        if self.vector_arguments['x']:
            vector_elements['x'] = self.centerx[idx,:num_points]

        if self.vector_arguments['y']:
            vector_elements['y'] = self.centery[idx,:num_points]

        if self.vector_arguments['radius']:
            vector_elements['radius'] = self.radii[idx,:num_points]

        if self.vector_arguments['intensity']:
            vector_elements['intensity'] = self.disk_intensities[idx,:num_points]
        ...
        if self.num_planes > 1 and (self.image_arguments['image'] or self.image_arguments['pixel_weights']):
            ...
            for i in range(self.num_planes):
                pix_w8 = None #if not self.image_arguments['pixel_weights'] else self.pixel_weights[idx, self.z_indices[idx,:num_points] == i]
                bg_w8 = self.background_weight * (1 if pix_w8 is None else np.min(pix_w8))
                img, wt_img = draw_disksW(self.shape[1:],
                                          coords[self.z[idx,:num_points] == self.z_values[idx, i], :],
                                          self.radii[idx,:num_points][self.z[idx,:num_points] == self.z_values[idx, i]],
                                          self.disk_intensities[idx,:num_points][self.z[idx,:num_points] == self.z_values[idx, i]],
                                          pix_w8,
                                          bg_w8)
                image[i, :, :] = img
            ...
            if self.vector_arguments['z']:
                vector_elements['z'] = self.z[idx,:num_points]

        else:
            image = np.zeros(self.shape, dtype=np.uint8)
            weight_image = np.zeros(self.shape, dtype=np.float32)
            pix_w8 = None if not self.image_arguments['pixel_weights'] else self.pixel_weights[idx,:num_points]
            bg_w8 = self.background_weight * (1 if pix_w8 is None else np.min(pix_w8))
            img, wt_img = draw_disksW(self.shape[1:],
                                              coords,
                                              self.radii[idx,:num_points],
                                              self.disk_intensities[idx,:num_points],
                                              pix_w8,
                                              bg_w8)
            image[0, :, :] = img
    ...
    def get_batch(self, batch_size):
      num_points = None
      if isinstance(self.n_points, list): num_points = np.random.randint(self.n_points[0], self.n_points[1])
      batch_coords = []
      batch_label = []
      for i in range(batch_size):
        data = self.__getitem__(self.count, num_points=num_points)
        self.count += 1
        if self.count >= self.__len__():
          if self.refresh: self.on_epoch_end()
          self.count = 0
        batch_coords.append(data[0])
        batch_label.append(data[1])
      return (np.array(batch_coords), np.array(batch_label))
...
if fixed_num_points: n_points = 10 #50
else: n_points = [5, 100]

num_steps_per_epoch = 100 // batch_size #N // batch_size

if fixed_radius_and_intensity: radius = 15 #[5, 25]
else: radius = [5, 25]

if fixed_radius_and_intensity: intensity = 1#[1, 5]
else: intensity = [1, 5]
...
decoder_channels = [1024, 512, 256, 128, 64]#[2048, 1024, 512, 256, 128, 64]
...
gen = RandoDotGenerator(N = N,
                        ...
                        output_arguments = {'x': True,
                                            ...
                                            'pixel_weights': False},
                       z_fixed=z_planes,
                       refresh=not fixed_training_set)
print(f"Refresh = {gen.refresh}")

test_gen = RandoDotGenerator(N = N_test,
                        n_points = n_points,
                        radius = radius,
                        intensity = intensity,
                        shape = shape,
...
def normalize(x):
    y = x - jnp.array([0, 0, min(z_planes), radius[0], intensity[0]])[None, None, ...]
    y /= jnp.array([shape[2], shape[1], max(z_planes) - min(z_planes), radius[1] - radius[0], intensity[1] - intensity[0]])[None, None, ...]
    y -= 0.5
    return y * 2
...
class DeepCGHEncoder(nn.Module):
  num_features: int = embed_size
  
  @nn.compact
  def __call__(self, x):
    embedding = x
    for i in range(6):
        embedding = nn.Dense(features=self.num_features)(embedding)
    ...
    if use_max: return embedding.max(axis=-2)
    else: return embedding.mean(axis=-2)
...
class FourierDecoder(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, train: bool):
        x = nn.Dense(int(prod(self.initial_shape)))(x)
        x = jnp.reshape(x, (-1, *self.initial_shape))

        x = fouriernet.FourierNet2D(fourier_features=self.fourier_features)(x)

        for cp in self.conv_params:
            x = nn.Conv(cp['ch'], kernel_size=cp['ks'], padding='SAME')(x)
            x = nn.relu(x)
...
class DeepCGH(nn.Module):
  ...
  initial_shape: tuple = (8, 8, 2048)

  def setup(self):
    self.channels = get_coord_channels((1, 128, 128))
    self.encoder = DeepCGHEncoder()
    self.decoder = DeconvDecoder(list(self.decoder_channels),
                                     self.initial_shape,
                                     self.channels)
  ...
  def __call__(self, coords, z_planes, train: bool):
    inputs = normalize(coords)
    encoded = self.encoder(inputs)

    decoded = self.decoder(encoded, train)
    slm_phase = jnp.angle(jnp.fft.ifft2(decoded[:,:,:,0] * jnp.exp(1j*2*jnp.pi*decoded[:,:,:,1])))

    recon = self.get_image_from_phase(slm_phase, z_planes)
    
    return (recon, slm_phase), (decoded[:,:,:,0], decoded[:,:,:,1], None)
...
def create_train_state(module, rng, learning_rate):
    ...
    if isinstance(n_points, list): number_of_points = max(n_points)
    
    variables = module.init(rng,
                            jnp.ones((batch_size, number_of_points, 5)),
    ...
    if starting_batch_stats_path is None:
        batch_stats = variables['batch_stats']
    else:
        print('Using starting batch stats')
        batch_stats = deserialize('', starting_batch_stats_path)
...
@jax.jit
def compute_test_metrics(*, state, batch):
    (recon, slm_phase), (amp, phase, field) = state.apply_fn({'params': state.params, 'batch_stats': state.batch_stats},
                                    coords = batch[0],
                                    z_planes = np.array(z_planes),
                                    train=False)
    ...
    metric_updates = state.test_metrics.single_from_model_output(predictions = recon, targets=batch[1], loss_cosine=loss['loss_cosine'], loss_acc=loss['loss_acc'])
    metrics = state.test_metrics.merge(metric_updates)
    state = state.replace(test_metrics=metrics)
...
for step in range(num_steps_per_epoch*epochs):
  ...
  if (step+1) % num_steps_per_epoch == 0: # one training epoch has passed
    ...
    if ((step+1) // num_steps_per_epoch) % 1000 == 0:
      for test_batch_index in range(100):
        test_batch = test_gen.get_batch(batch_size)
        state, cache_test = compute_test_metrics(state=state, batch=test_batch)
        if ((step+1) // num_steps_per_epoch) % 10000 == 0:
            serialize(save_path, f'cache_test_{test_batch_index}_epoch_{(step+1) // num_steps_per_epoch}.pkl', cache_test)
        serialize(save_path, f'cache_test_{test_batch_index}.pkl', cache_test)
      print(f'Saved cache test to cache_test_i.pkl and cache_test_i_epoch_{(step+1) // num_steps_per_epoch}.pkl for i in range(100)')

      for metric,value in state.test_metrics.compute().items(): # compute metrics
        metrics_history[f'test_{metric}'].append(value) # record metrics

      state = state.replace(test_metrics=state.test_metrics.empty()) # reset test_metrics for next training epoch
    ...
    if ((step+1) // num_steps_per_epoch) % 10000 == 0:
      ...
      serialize(save_path, f'batch_stats_epoch_{(step+1) // num_steps_per_epoch}.pkl', state.batch_stats)
      print(f'Saved batch stats to batch_stats_epoch_{(step+1) // num_steps_per_epoch}.pkl')
    ...
    if ((step+1) // num_steps_per_epoch) % 50 == 0:
      ...
      serialize('', f'batch_stats_current.pkl', state.batch_stats)
      print(f'Saved batch stats to batch_stats_current.pkl')
    ...
    get_last_element = lambda arr: None if len(arr) == 0 else arr[-1]
    print(f"train epoch: {(step+1) // num_steps_per_epoch}, "
          f"loss_acc: {get_last_element(metrics_history['train_loss_acc'])}, "
          f"loss_cosine: {get_last_element(metrics_history['train_loss_cosine'])}, "
          f"test_loss_acc: {get_last_element(metrics_history['test_loss_acc'])}, "
          f"test_loss_cosine: {get_last_element(metrics_history['test_loss_cosine'])}, "
          )
  t4 = time.time()
...
serialize(save_path, f'batch_stats_{datetime}.pkl', state.batch_stats)
print(f'batch_stats_{datetime}.pkl')
